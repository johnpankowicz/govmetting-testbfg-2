...
Information on the _SECRETS files is in the setup instructions for <a href="about?id=setup#GoogleCloud"> Google Cloud Platform </a> and <a href="about?id=setup#GoogleApi"> Google API Keys. </a> You will only need these files if you are working on speech-to-text  transcription or on user login.
