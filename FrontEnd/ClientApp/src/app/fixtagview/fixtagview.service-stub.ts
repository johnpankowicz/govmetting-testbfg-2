...
import { FixTagViewSample } from './fixtagview-sample';

  const fromFile = true;
  const url = 'assets/stubdata/ToFixTagView.json';
  // Use the jsonplaceholder service to test post requests
  const addtagsUrl = 'https://jsonplaceholder.typicode.com/posts'
  const NoLog = true;  // set to false for console logging

  @Injectable()
export class FixtagviewServiceStub {
  ...
  public constructor(private http: HttpClient, private errHandling: ErrorHandlingService) {
    ...
    public getTalks(): Observable<FixTagView> {
      if (fromFile) {
